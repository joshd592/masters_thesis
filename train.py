...
import sys
from globals import printt, workingDir
from globals import ScenarioDataset, FromSequenceModel, FromStateModel, NotTooLowLoss
blnPlot = False
if blnPlot:
    plt.ioff()
    fig, ax = plt.subplots()
...
#Hyperparameters
seqLength = 3 #Anything over 1 switches models
localState = False
val_size = 1500
batch_size = 2 ** 9
# hidden_size = 2 ** 11
...
hidden_sizes = [2**p for p in range(9,12)]
num_hidden_layerss = range(5,7)

datapath = workingDir + "gridsweep.h5"
print("Saving log as: ", time.strftime('locallogs/sweep_log_%b%d_%H%M.txt'))
with open(time.strftime(workingDir + '/locallogs/sweep_log_%b%d_%H%M.txt'), "w") as f:
    for (hidden_size, num_hidden_layers) in itertools.product(hidden_sizes, num_hidden_layerss):
        ...
        normFromFile = torch.load(workingDir + '/normsfor'+datapath[:datapath.find(".")]+'.pth', weights_only=True)
        ...
        if seqLength == 1:
            ...
            model = FromSequenceModel(input_size=dims, hidden_size=hidden_size, seqLength=seqLength)
        ...
        min_val_loss=np.inf
        update = False
        model_path = time.strftime(workingDir + '/localsaves/model' + str(seqLength) + '_%b%d_%H%M.pth')
                ...
                for batchidx, (state, tback) in enumerate(train_loader):
                    ...
                    if (batchidx + 1) % 5 == 0:
                        ...
                        if val_loss < min_val_loss:
                            ...
                            print(f"Epoch {epoch}, Batch {batchidx+1} | sqrtTrain: {np.sqrt(loss.item()):.4f}, sqrtVal: {np.sqrt(val_loss):.4f} | LR: {optimizer.param_groups[0]['lr']:.2e} saved")
                        else:
                            print(f"Epoch {epoch}, Batch {batchidx+1} | sqrtTrain: {np.sqrt(loss.item()):.4f}, sqrtVal: {np.sqrt(val_loss):.4f} | LR: {optimizer.param_groups[0]['lr']:.2e}")
                        if blnPlot:
...
if __name__ == '__main__':
    import time
    checkpoint = torch.load(workingDir + '/localsaves/bestlstm.pth', weights_only=False)
    ...
    if sequenceLen == 1:
        ...
        bestmodel = FromSequenceModel(input_size=dims, hidden_size=hidden_size, num_hidden_layers=hidden_size)
        bestmodel.load_state_dict(state_dict)
        bestmodel.eval()
    ...
    # bestmodel.eval()
    normFromFile = torch.load(workingDir + '/norm.pth', weights_only=True)
    tback_min = normFromFile['tback_min']
    tback_max = normFromFile['tback_max']
    state_min = normFromFile['state_min_loc'] if localState else normFromFile['state_min_abs']
    state_max = normFromFile['state_max_loc'] if localState else normFromFile['state_max_abs']
    ...
    dataset_test = ScenarioDataset(workingDir + '/big2.h5', sequence_length=1, local=localState)
    torch.manual_seed(time.time())
